...
foreach my $l (@fr){ # Begin foreach my $l (@fr)
	...
	if($l=~m/^[\t\ ]{0,}$/){ # Begin if($l=~m/^[\t\ ]{0,}$/)
		...
		if($l =~ m/([^=]+)=(.+)/){ # Begin if($l=~ m/[\t\ ]*=[\t\ ]*/)
			...
			$rn=~s/^[\t\ ]{0,}//g;# We remove invisible chararter at the begining of the string
			$rn=~s/[\t\ ]{0,}$//g;# We remove invisible chararter at the end of the string
			if($rn !~ m/^${id_rn}$/){ # Begin if($rn !~ m/^${id_rn}$/)
				print "Line $lnum the rule name $rn format not accepted.\n";
				exit(-1);
			} # End if($rn !~ m/^${id_rn}$/)
		} # End if($l=~ m/[\t\ ]*=[\t\ ]*/)
		my($rn,$rd)=($trn,$trd); # rule name,rule delaration reinitialised
		if(length($rd)==0){ # Begin if(length($rd)==0)
			# We check if line declare rule
			if($l!~m/		# Does not match to the regular expression
				  ^[\ \t]{0,}	# Line starts with space or tab 0 or +infinite times
		...
		$rn=~s/[\ \t]{1,}//g; # We remove consecutive invisible character from $rn (we repeat it each time pattern exists)
		$rd=~s/[\ \t]{1,}/ /g; # We remove cosecutive invisible character from $rd and replace them with one space (we repeat it each time pattern exists)
			...
			if($rn!~m/\>$/){ # Begin if($rn!~m/\>$/)
				if($rn!~/^\"[^\"]+\"$/){ # Begin if($rn!~/^\"[^\"]+\"$/)
					...
					if(!defined($osbr{"$rn"})){ # Begin if(!defined($osbr{"$rn"}))
						$osbr{"$rn"}="rn"; # We create the reference in the hash but body is not yet defined
