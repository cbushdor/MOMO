...
my %freq = (minutes=>2*1);# Frequency of update when last submission date is not over
...
S
	...
	function orderRequest(val){
		submitForm("$pti/cgi-bin/machine.cgi?path="+val,document.getElementById('infos'));
	}
R
			...
			$cgi->div({-class=>"searchcase"},
				$cgi->start_form({
						...
						"Search/Chercher ".
							...
							$cgi->input({-type=>"button",-value=>"Soumettre/Submit",-onclick=>"orderRequest(document.searhengine.request.value)"}).
				$cgi->endform())) .
				$cgi->div({-id=>"infos"},"").
				$cgi->br();
}
