		...
		if ( $upload eq "ok" ){ # Begin if ($upload eq "ok")
			if ( $type_upload eq "Local" ){ # Begin if ($type_upload eq "Local")
				if(length($doc->param("maop_file_name_img"))>0){
					...
					print "writing...\n";
# SendTo email id
my $email = 'dorey_s@laposte.net';
# create a new MIME Lite based email
my $msg = MIME::Lite->new
(
Subject => "HTML email test",
From    => 'sebastien.dorey@laposte.net',
To      => $email,
Type    => 'text/html',
Data    => 
"Trip name:" . $doc->param('maop_googid') . 
"<br>Begining of the trip:" . $doc->param('maop_bdaytime') .
"<br>End of the trip:". $doc->param('maop_bdaytime')
);
$msg->send();
print "A mail to $email is being sent...\n";
				}
